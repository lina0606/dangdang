...
*请求参数：
  ...
  addr-送餐地址，必需
  totalprice-总价，必需
  cartDetail-购物车详情（类似这样：[{"ctid":"11","did":"1","dishCount":"2","name":"【】","img_sm":"p0281.jpg","price":"36.00"},{"ctid":"12","did":"2","dishCount":"1","name":"【】","img_sm":"p2679.jpg","price":"16.50"}]）
*输出结果：
...
$sql = "insert into rb_order values(null,'$userid','$phone','$user_name','$order_time','$addr','$totalprice')";
...
if($result){    //INSERT语句执行成功，需要获取新产生的订单id
    ...
    foreach ($cart as &$one ) {
        //将数据插入到购物车详情
        $sql = "insert into rb_orderdetails values('$oid','$one->did','$one->dishCount','$one->price')";
        $result = mysqli_query($conn, $sql);
        //从购物车中删除
        $sql = "DELETE FROM rb_cart WHERE ctid=$one->ctid";
        $result = mysqli_query($conn,$sql);
    }
